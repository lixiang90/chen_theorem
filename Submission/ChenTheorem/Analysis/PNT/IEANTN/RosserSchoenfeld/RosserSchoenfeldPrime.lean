/-
Adapted from PrimeNumberTheoremAnd/IEANTN/RosserSchoenfeld/RosserSchoenfeldPrime.lean
...
import Submission.ChenTheorem.Analysis.PNT.MediumPNT
...
lemma pre_413_measure_inter {x : ℝ} (hx : 2 ≤ x) (y : Finset.Ico 1 ⌊x⌋₊) :
  ...
  · simp only [y_eq, cast_one, reduceAdd]
    ...
    simp [Real.log_nonneg, Nat.prime_two]
...
theorem eq_413 {f : ℝ → ℝ} {x : ℝ} (hx : 2 ≤ x) (hf : ∀ t ∈ Set.Icc 2 x, DifferentiableAt ℝ f t)
  ...
  let a : ℕ → ℝ := Set.indicator {n : ℕ | Nat.Prime n} (fun n ↦ log n)
