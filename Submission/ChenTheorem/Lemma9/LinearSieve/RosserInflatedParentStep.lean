import Submission.ChenTheorem.Lemma9.LinearSieve.RosserInflatedStep
...
`z + 1`. This is a conditional active step, uniform in both depths. -/
theorem rosserPartialDefect_inflated_upper_parent_step (d δ : ℝ) (hd : 3 < d)
    (hδ : 0 ≤ δ) (hδ1 : δ < 1) (hgap : 3 / d < 1 - δ) :
    ∀ᶠ D : ℝ in atTop, ∀ R N : ℕ, ∀ M : ℝ, 1 ≤ M → ∀ z : ℕ,
      ...
      rosserPartialDefect P (R + 1) (z + 1) true D ≤
        upperContinuousPartial N (sieveParameter D (z + 1)) +
  ...
  filter_upwards [rosserPartialDefect_inflated_upper_step d δ hd hδ hδ1 hgap,
  ...
  intro R N M hM z hwz hs P hP hodd hactive hchild
  ...
  have h := hstep R N M hM z hwz hsz P hP hodd hactive hchild
  ...
  have hm := antitoneOn_mul_upperContinuousPartial N
    (show (0 : ℝ) < sieveParameter D (z + 1) by linarith)
    (show (0 : ℝ) < sieveParameter D z by linarith) hst
  have hnonneg := upperContinuousPartial_nonneg N (sieveParameter D (z + 1)) (by linarith)
  have hmul := mul_le_mul_of_nonneg_right hst hnonneg
  have hmain : upperContinuousPartial N (sieveParameter D z) ≤
      upperContinuousPartial N (sieveParameter D (z + 1)) := by nlinarith
...
/-- All prefix and density losses are absorbed at the actual parent cutoff
`z + 1`. This is a conditional active step, uniform in both depths. -/
theorem rosserPartialDefect_inflated_lower_parent_step (d δ : ℝ) (hd : 3 < d)
    (hδ : 0 ≤ δ) (hδ1 : δ < 1) (hgap : 3 / d < 1 - δ) :
    ∀ᶠ D : ℝ in atTop, ∀ R N : ℕ, ∀ M : ℝ, 1 ≤ M → ∀ z : ℕ,
      ...
      rosserPartialDefect P (R + 1) (z + 1) false D ≤
        lowerContinuousPartial (N + 1) (sieveParameter D (z + 1)) +
        M * ((1 - (1 - δ) / (64 * growingSieveParameter d (Real.log D))) *
          inflatedAuxiliaryError d δ D (sieveParameter D (z + 1)) lowerAuxiliaryError) := by
  have hg := (tendsto_growingSieveParameter_atTop d (by linarith)).comp Real.tendsto_log_atTop
  filter_upwards [rosserPartialDefect_inflated_lower_step d δ hd hδ hδ1 hgap,
  ...
  have hcoef : 0 ≤ 1 - (1 - δ) / (32 * growingSieveParameter d (Real.log D)) := by
    ...
    linarith
  have he := mul_le_mul_of_nonneg_left (mul_le_mul_of_nonneg_left hEt hcoef) (by linarith : 0 ≤ M)
  have hm := antitoneOn_mul_lowerContinuousPartial (N + 1)
    (show (0 : ℝ) < sieveParameter D (z + 1) by linarith)
    (show (0 : ℝ) < sieveParameter D z by linarith) hst
  have hnonneg := lowerContinuousPartial_nonneg (N + 1) (sieveParameter D (z + 1)) (by linarith)
  have hmul := mul_le_mul_of_nonneg_right hst hnonneg
  have hmain : lowerContinuousPartial (N + 1) (sieveParameter D z) ≤
      lowerContinuousPartial (N + 1) (sieveParameter D (z + 1)) := by nlinarith
  have hsmall : 0 ≤ (((1 - δ) / 64) / growingSieveParameter d (Real.log D)) *
