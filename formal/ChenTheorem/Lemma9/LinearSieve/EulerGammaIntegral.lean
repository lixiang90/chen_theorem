/-
The two Gamma integral proofs below are adapted from the local Apache-2.0
vendored module `ChenTheorem.Analysis.PNT.IEANTN.Mertens` (Issue1584).
See `ChenTheorem/Analysis/PNT/LICENSE` and `NOTICE` for attribution.
-/
...
theorem integral_log_mul_exp_neg_eq_deriv_Gamma :
...
theorem integral_log_mul_exp_neg_eq_neg_eulerMascheroni :
