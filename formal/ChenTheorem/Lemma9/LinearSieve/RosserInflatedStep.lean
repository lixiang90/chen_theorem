...
import ChenTheorem.Lemma9.LinearSieve.InflatedAbsorbedChildSum
import ChenTheorem.Lemma9.LinearSieve.ContinuousTerminalAbsorption

open Filter Finset

namespace Chen.LinearSieve

/-- The active cumulative-depth step with both density losses absorbed.
The threshold is uniform in the depths and multiplier. The actual small-prime
prefix and the child induction hypothesis are retained explicitly. -/
theorem rosserPartialDefect_inflated_upper_step (d δ : ℝ) (hd : 3 < d)
  ...
  rw [hparam] at hsB
  have hb' := hb N D (growingPrefixCutoff d D) z hD hw hwz hs P hP hodd
...
The threshold is uniform in the depths and multiplier. The actual small-prime
prefix and the child induction hypothesis are retained explicitly. -/
theorem rosserPartialDefect_inflated_lower_step (d δ : ℝ) (hd : 3 < d)
    (hδ : 0 ≤ δ) (hδ1 : δ < 1) (hgap : 3 / d < 1 - δ) :
    ∀ᶠ D : ℝ in atTop, ∀ R N : ℕ, ∀ M : ℝ, 1 ≤ M → ∀ z : ℕ,
